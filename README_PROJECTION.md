...
用 AI +  为自己和家人搭建私有化健康管理系统。

让你的 AI（Claude / ChatGPT / Gemini）读一遍，它就能指引家庭健康档案。

系统输入包括：体检报告照片、药盒照片、体检档案、Apple Watch 数据分析、科室名 + 历史档案

系统输出包括： 
1. 微信版口语化 （把分析结果转成父母能看懂的话）：饮食禁忌 + 运动建议 + 用药提醒 + 就医信号
2. 复查日历生成 ：生成复查时间表，标注过期/即将到期项
